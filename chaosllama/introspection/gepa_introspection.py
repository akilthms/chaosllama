...
from chaosllama.services.genie import GenieAgent ,GenieAgent_v2, GenieService
...
def get_eval_set():
    return spark.table(f"{config.CATALOG}.{config.SCHEMA}.{config.EVAL_TABLE_NAME}").toPandas()
...
class DspyGenieAdapter(Adapter):
    ...
    def __call__(
        ...
        print("Signature to DspyGenieAdapter: ", signature.instructions)
        print("Inputs to DspyGenieAdapter: ", inputs)
        return lm(prompt=signature.instructions, messages=inputs["question"],**lm_kwargs)

class DspyGenieLM(BaseLM):
    def __init__(self, genie_space_id, *args, **kwargs):
        super().__init__(None,**kwargs)
        self.genie_space_id = genie_space_id
        self.genie_agent = GenieAgent(config,timeout=60)
...
class DspyIntrospectionSignature_v2(dspy.Signature):
    ...
    genie_sql_query: str = dspy.OutputField(desc="Generated SQL Query from Genie")
...
class DspyIntrospection(dspy.Module):
    def __init__(self,genie_space_id: str, ):
        ...
        self.introspect = dspy.Predict(DspyIntrospectionSignature_v2, provide_traceback=True)  # dspy.ChainOfThought(DspyIntrospectionSignature)

    def forward(self, question: str, provide_traceback=True):
...
class DspyIntrospectionAgent(IntrospectiveAI):
    ...
    def __init__(self, agent_config, judges_service:JudgeService, genie_service:GenieService, eval_mngr: EvalSetManager=None,**kwargs):
    ...
    def gepa_introspection(self, config:dict):
        ...
        gepa = dspy.GEPA(
            ...
            #auto="light",
            reflection_minibatch_size=config.get("reflection_minibatch_size", 1),
        ...
        with mlflow.start_run(run_name=f"Test Tracing + {time.time()}"):
    ...
    @staticmethod
    def calc_composite_metric_v2(feedbacks: List[DspyFeedback], weight_map: dict=None) -> float:

        if weight_map is None:
            # TODO: Make this more robust, to adjust for developer supplied custom scorers / guidelines.
            weight_map = {
                "sql_results_equivalence": 0.33,
                "sql_clauses_distribution_equivalence": 0.33,
                "sql_semantic_equivalence": 0.33
            }

        composite_metric = sum([ bool(f.score)*weight_map[f.name] for f in feedbacks])
    ...
    def calc_composite_metric(metrics: dict,
        ...
        chaosllama_metrics_quality_score = CHAOSLLAMA_METRICS_WEIGHT * DspyIntrospectionAgent.calc_quality_score(_metrics,
                                                                                               weighted_chaosllama_metrics_map)

        custom_metrics_quality_score = CUSTOM_METRICS_WEIGHT * DspyIntrospectionAgent.calc_quality_score(_metrics,
    ...
    def get_guidelines(self) -> dict:
        return config.scorers.global_guidelines[config.scorers.RUNTIME_VERSION]
    ...
    def collect_feedback(evalset: MosaicJudgesInput,
        ...
        for name, guideline in guidelines.items():
            ...
            match type(result.value):
                case "CategoricalRating":
                    score = 0 if (result.value == CategoricalRating.NO) else 1
                case "float" | "int":
                    raise ValueError("Chaos Llama custom scorers does not support returning float or int values. Please return boolean values only.")
                case _:
                    score = result.value

            feedbacks.append(
                DspyFeedback(name=name , score=score, feedback=result.rationale)
            )
    ...
    def get_dspy_metric(self,
        ...
        print(f"📊Getting DSPY METRIC: {example}")
        ...
        scorers = get_judges() #self.jmngr.scorers
        guidelines = None #self.get_guidelines()
        feedbacks = self.collect_feedback(formatted_evalset, scorers, guidelines)

        composite_metric = self.calc_composite_metric_v2(feedbacks)
...
def get_judges():
    ...
    judge_list = jmngr.scorers # jmngr.judges
    return judge_list
...
def test_dspy_introspection():
    ...
    exp_mngr = MLFlowExperimentManager(experiment_path=config.mlflow.MLFLOW_EXPERIMENT_PATH)

    exp_mngr.set_experiment(f"{config.mlflow.MLFLOW_EXPERIMENT_PATH}{config.mlflow.MLFLOW_GEPA_EXPERIMENT}")
